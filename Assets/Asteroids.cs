...
public class Asteroids : MonoBehaviour
{
	...
	GameObject[] bullets = new GameObject[2];       // Bullet 0 is player's, 1 is flying saucer's
	...
	void Start()
	{
		...
		saucer.layer = 5;
		saucerTime = Time.time + 30f;// + Random.value * 60f;
		for (int i = 0; i < (2 + level * 2); i++)
		{
			GameObject asteroid = CreateVectorObject(RandomDirection().ToString(), RandomAsteroidShape(), 0.4f);
	...
	void Update()
	{
		if (!gameOver)
		{
			for (int i = 0; i < asteroids.Count; i++)
				...
				{
					int dir = int.Parse(asteroids[i].name);
					...
					if ((hits != null) && (hits.Length > 0))
					{
						...
						else if (hits[0].gameObject.layer >= 2)
						{
							if (hits[0].gameObject.layer < 4)
								for (int p = 0; p < 2; p++)
								{
									GameObject asteroid = CreateVectorObject(RandomDirection().ToString(), RandomAsteroidShape(), hits[0].gameObject.layer == 2 ? 0.2f : 0.1f);
		...
		else
		{
			if (Input.GetKeyDown(KeyCode.Space))
			{
				score = 0;
				lives = 2;
				UnityEngine.SceneManagement.SceneManager.LoadScene("Asteroids");      // Reload scene and reset score & lives for a new game
			}
		}
